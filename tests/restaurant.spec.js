...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // considerando que a função createMenu() foi chamada com o objeto: `{ food: {}, drink: {} }`.
     ...
     expect(objetoRetornado1.fetchMenu()).toHaveProperty('food');
     expect(objetoRetornado1.fetchMenu()).toHaveProperty('drink');
    ...
    // TESTE 4: Verifique se 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
      ...
      expect(objetoRetornado1.consumption).toEqual([]); 
    ...
    // ```
       ...
       objetoRetornado1.order("Entrecot");
       objetoRetornado1.order("sopa");
       objetoRetornado1.order("sashimi"); 
       expect(objetoRetornado1.consumption).toEqual(["coxinha","Coca", "Entrecot", "sopa", "sashimi"]);
    // ```
    ...
    // TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
      const newOrder1 = createMenu({food: {'Coxinha': 5.00, 'Entrecot': 35.00}, drink: {'Agua': 3.50, 'Coca': 7.00}});
    //
       newOrder1.order('Coxinha');
       newOrder1.order('Agua');

       expect(newOrder1.pay()).toBeCloseTo(9.35);
